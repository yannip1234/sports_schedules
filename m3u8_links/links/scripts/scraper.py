...
class BTS:
    def __init__(self, category):
        ...
        self.games = []
        self.scrape(self.url_root, self.url_init, self.url_category)
    def links(self):
        for game in self.games:
            print(game.title)
            print(game.link)

    def scrape(self, url_root, url_init, url_category):
        soup = BeautifulSoup(r.get(url_root + url_init + url_category).text, features='html.parser')
        ...
        for game in games:
            ...
            url = url_root + url_directory.get('href')
            headers = {
